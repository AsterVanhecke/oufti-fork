...
for k = 1:numberOfSpots.NumObjects
    ...
    try
        ...
        % Dilate mask
        tempMask = imdilate(tempNewRawImage>0,SE);
        ...
        maxBackgroundEstimate=mean(rawImage(:))+std(double(rawImage(:)));
    catch
        continue;
    end
        ...
        switch maxIndex
            case 1
                ...
                if (sfitTemp.heightEstimate/65535) < postFitMinHeight || ((sfitTemp.heightEstimate/65535) > 2 ) || ...
                        ...
                        dilatedCellContour(:,1),dilatedCellContour(:,2)) ...
                        || sfitTemp.positionX<min(columnPosition) || sfitTemp.positionX>max(columnPosition) || ...
                        sfitTemp.positionY<min(rowPosition) || sfitTemp.positionY>max(rowPosition)
                    continue;
                end
                counter = counter + 1;
                try
            ...
            case 2
                ...
                for jj = 1:2
                    if (h(jj)/65535) < postFitMinHeight || ((h(jj)/65535) > 1 ) || ...
                            ...
                            dilatedCellContour(:,1),dilatedCellContour(:,2)) ...
                            || posX(jj)<min(columnPosition) || posX(jj)>max(columnPosition) || ...
                            posY(jj)<min(rowPosition) || posY(jj)>max(rowPosition)
                        continue;
                    end
