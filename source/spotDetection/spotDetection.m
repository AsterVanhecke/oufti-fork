...
try % loading meshData from cellList
       ...
       if exist('cellList','var')==1
          ...
          for cell = cellRange1:cellRange2
           ...
           if showMesh
            ...
            for i=1:length(mgn)
                img2(min(size(img,1),max(1,round(ypos(i)))),min(size(img,2),max(1,round(xpos(i))))) = mgn(i)*intmax(class(img)); %%% -was commented
